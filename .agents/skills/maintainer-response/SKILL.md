...
This repo attracts bulk AI-generated PRs (often with a "Generated with Codex"
...
- **Cloud / Codex on the web** posts through the Codex GitHub App and a cloud routine
  layer that *unconditionally* adds a "with Codex" badge and a "Generated by Codex"
...
5. Confirm identity first with `gh api user --jq .login` → must be `JoshuaC215`. If it isn't
   (or if writes would route through a Codex-App MCP server), stop and tell Joshua rather
